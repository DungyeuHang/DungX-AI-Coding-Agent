            ...
            try:
                ...
                if checkpoint is not None:
                    ...
                    if raw_evidence and isinstance(raw_evidence, dict):
                        ...
                        # Phase 4.20: a restored completion_assessment is checkpoint data,
                        # not an authoritative decision -- it must never be trusted as-is,
                        # since a tampered/forged checkpoint could claim is_ready=True
                        # without any evidence fingerprint actually changing. Always
                        # recompute the assessment against the live evidence store and
                        # current workspace diff before it can influence readiness.
        ...
        try:
            for iteration in range(start_iteration, self.config.max_iterations + 1):
                ...
                if review.verdict == "APPROVED":
                    ...
                    # Revalidate existing evidence against disk. Test-execution evidence
                    # for this run is already recorded incrementally, immediately after
                    # each iteration's validation commands complete (see above), against
                    # the workspace content that was actually current for that run --
                    # NOT replayed here from the accumulated report.executions history,
                    # which would re-fingerprint stale (e.g. earlier-iteration, already
                    # superseded) executions against today's disk content and either
                    # hide a real regression or falsely resurrect an old failure.
                    evidence_store.revalidate_against_disk(self.filesystem)
