...
class MockTaskStorage(TaskStorage):
    ...
    def load_scheduler_state(self):
        from local_agent.models import SchedulerState
        return self.scheduler_state or SchedulerState()

    def save_provider_configs(self, configs) -> None:
        self.provider_configs = list(configs)

    def load_provider_configs(self) -> list:
        return list(self.provider_configs)

    def save_semantic_index(self, index) -> None:
        self.semantic_index = index

    def load_semantic_index(self):
        from local_agent.models import SemanticIndex
        return self.semantic_index or SemanticIndex()

    def save_project_memory(self, memory) -> None:
        self.project_memory = memory

    def load_project_memory(self):
        from local_agent.models import ProjectMemory
        return self.project_memory or ProjectMemory()
...
class CapturingProvider(AIProvider):
    ...
    def generate_code(self, task: str, plan: Plan, context: ProjectContext, failure: FailureAnalysis | None = None, review: ReviewResult | None = None) -> list[FileOperation]:
        ...
        return [FileOperation("modify", "src/app.py", content="def hello(): return 'world'\n", reason="Updated app")]
...
class Phase39Tests(unittest.TestCase):
    ...
    def _make_orchestrator(self, provider: CapturingProvider) -> Orchestrator:
        orch = Orchestrator(self.config, self.storage, None, self.repo_lock, self.memory_lock)
        return orch
    ...
    def test_task_reload_after_process_restart_simulation(self):
        task_objective = "Implement a new feature"
        provider = CapturingProvider()
        orchestrator1 = self._make_orchestrator(provider)
        task = self._create_task_with_plan(task_objective)
        with mock.patch("local_agent.orchestrator.build_provider", return_value=provider):
        ...
        orchestrator2 = self._make_orchestrator(provider)
    ...
    def test_quota_exhaustion_leads_to_paused_state_and_resume(self):
        ...
        orchestrator1 = self._make_orchestrator(provider1)
        task = self._create_task_with_plan(task_objective)
        with mock.patch("local_agent.orchestrator.build_provider", return_value=provider1):
            report1 = orchestrator1.run(task, "sub1")
        ...
        orchestrator2 = self._make_orchestrator(provider2)
        with mock.patch("local_agent.orchestrator.build_provider", return_value=provider2):
